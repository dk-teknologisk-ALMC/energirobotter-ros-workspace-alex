...
# SSH ASKPASS helper
# ----------------------------------------------------------------------
# SSH kan ikke prompte for password fra en non-TTY subprocess. I stedet
# bruger den $SSH_ASKPASS — en eksekverbar fil der printer password til
# stdout. Vi genererer en zenity-wrapper i /tmp ved opstart, og spawner
# SSH-services med SSH_ASKPASS_REQUIRE=force + stdin=DEVNULL.
...
#
# Uden multiplexing skal hver 'ssh -tt' autentificere separat — et
# password-prompt pr. service og pr. animation. ControlMaster lader
# foerste ssh oprette en master-socket; efterfoelgende ssh'er deler
# socketen og springer auth over. ControlPersist holder masteren oppe
# i 10 min efter sidste forbindelse.
#
# Permanent loesning er ssh-copy-id, men ControlMaster virker uden
# aendringer paa Jetson-siden og er sikkerhedsmaessigt identisk.
#
# ControlPath skal vaere kort (sun_path-limit paa 108 tegn).
...
#
# 'ssh -tt host "cmd"' koerer non-interaktivt og laeser ikke .bashrc
# (bash's '[[ $- == *i* ]] || return'-guard). Derfor sourcer vi de
# tre workspaces eksplicit her.
#
# Vi cd'er foerst til workspace-roden fordi flere noder paa Jetson
# (fx wattson_servo_manager_node) aabner config-filer via relative stier.
# Workspace ligger to steder afhaengigt af robot:
...
JETSON_SOURCES = (
    ...
    # zed_wrapper bor i en separat workspace pr. README'en.
    # NB: hele SSH-kommandoen wrappes i ' ' af kalderen, saa ingen enkelt-
    # anforselstegn maa optræde herinde.
...
SERVICES = [
    # --- Network / forudsætninger ---
    {
        ...
        "section": "Network",
        # 'sudo -A' bruger zenity-ASKPASS til at promte foerste gang.
        # Stop sker via SIGINT til hele subprocess-pgroup'en — sudo
        # videresender signalet til dnsmasq, saa root-processen lukker
        # uden ekstra auth.
        # Begge Jetson-MACs reserveres til .105 i samme --dhcp-host
        # (dnsmasq accepterer flere hwaddr pr. entry, men afviser flere
        # entries med samme IP). Kun een robot koeres ad gangen:
        #   48:b0:2d:eb:e3:58  - original Energinet robot Jetson
        #   ac:3a:e2:12:39:5f  - bench Jetson (Wattson, JP6.0)
        # --dhcp-authoritative + wipe af leases tvinger .105 selv ved
        # stale lease.
        "command": (
    ...
    },
    {
        ...
        "section": "Network",
        # Kommandoen er sekund-hurtig; wrappes i sleep-loop saa den
        # bliver "koerende" og Stop-knappen kan afbryde den.
        "command": (
            "adb devices"
    ...
    # Manuel slider-GUI til commissioning og enkelt-led-test.
    # joint_state_publisher_gui + RViz. Sliderne publicerer paa
    # /joint_states og driver dermed servoerne via Jetson-siden.
    #
    # ADVARSEL: konkurrerer med animation_player om /joint_states.
    # Stop animationer foer slider-brug, og omvendt.
    {
        "key": "slider_control",
    ...
    # Power-monitor-servicen er fjernet fra launcher_GUI'en. Servo-driveren
    # initialiseres med feedback_enabled=False og SyncRead af PRESENT_VOLTAGE/
    # PRESENT_CURRENT er deaktiveret for at undgaa ekstra roundtrip per cycle.
    # Power-noden kan stadig koeres separat via
    # 'ros2 run arm_commissioning power_monitor_node'.
]

CHECKLIST = [
    # ESP32-bokse er bundet til faste USB-porte via /dev/serial/by-path/,
    # saa plug-raekkefoelgen er ligegyldig, men de SKAL i den rigtige port.
    # Kameraet skal have koert mindst én gang foer bokse plugges i.
...
# til en CSV i energirobotter_bringup/animations/.
#
# Format: (csv_basename, dansk_label).
#
# Loop-flag udledes fra filnavnet via _csv_should_loop() — kun
# 'animation_*' og 'mimic_*' loopes. Alt andet afspilles én gang.
ANIMATIONS = [
    ("Sikre / blide", [
        ("idle1", "Idle (rolig)"),
        ("gesture_yes", "Yes (nik)"),
...
class AnimationRunner:
    ...
    def play(self, csv_basename, log_fn, loop=False):
        if self.is_running():
            ...
            self._kill()
            # SIGHUP via sshd-lukning er upaalideligt ved hurtig relaunch —
            # send eksplicit pkill over SSH (ControlMaster gor det <100ms).
            self._remote_kill(log_fn)
            # Kort grace saa servoer slipper før ny node forbinder
            time.sleep(0.4)

        # $(ros2 pkg prefix ...) ekspanderes af *remote* shell — lokal
        # single-quote forhindrer lokal ekspansion. 'exec' erstatter remote
        # bash med ros2-processen, saa SIGHUP rammer rclpy direkte naar
        # SSH-kanalen lukker (uden 'exec' overlevede noden 30+ sek).
        # 'loop:=' styrer om CSV'en wrapper uendeligt eller afslutter ved EOF.
    ...
    def stop(self, log_fn):
        if not self.is_running():
            # Selv hvis lokal proces er død kan en remote zombie stadig
            # kommandere servoer — send altid pkill for at vaere sikker.
            self._remote_kill(log_fn)
            return
...
class LauncherApp(tk.Tk):
    def __init__(self):
        ...
        # Log-batching: subprocess-linjer queues thread-safe og flushes i
        # bursts paa main-thread hver 50 ms. Uden batching kunne busy nodes
        # fylde tkinter event-queue'en og blokere UI-klik flere sekunder.
        # maxlen forhindrer ubegraenset hukommelses-vaekst.
